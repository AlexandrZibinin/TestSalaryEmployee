...
from src.csv_reader import reader_csv, read_list_csv
from src.report_generate import detect_salary_id, print_payouts_report
...
def test_read_list_csv_sorting_stability():
    ...
    with tempfile.NamedTemporaryFile(
        mode="w+", suffix=".csv"
    ) as f1, tempfile.NamedTemporaryFile(mode="w+", suffix=".csv") as f2:
...
def test_reader_csv_special_characters():
    """Тест данных с кавычками и запятыми"""
    content = '''name,department,pay\n"Иванов, Петр",IT,"1,000"'''

    with tempfile.NamedTemporaryFile(mode="w+", suffix=".csv") as f:
        f.write(content)
        f.flush()
        result = reader_csv(f.name)
        assert result[0]["name"] == "Иванов, Петр"


# tests.py
def test_negative_hours_worked():
    """Тест отрицательных часов работы"""
    data = [
        {"department": "HR", "name": "Alice", "hours_worked": "-10", "salary": "50"}
    ]

    with pytest.raises(ValueError):
        print_payouts_report(data)
